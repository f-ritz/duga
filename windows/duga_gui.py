...
Features added:
- Application icon support (icon.ico - place in project root)
...
class SystemTray:
    ...
    def __init__(self, app: "DugaApp", icon_path: Optional[str], title: str):
        self.app = app
    ...
    def _show_menu(self, hwnd):
        ...
        win32gui.AppendMenu(menu, win32con.MF_STRING, 1, "Show Duga")
...
class DugaApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title(f"Duga {APP_VERSION}")
    ...
    def _find_icon_path(self) -> Optional[str]:
        ...
        if getattr(sys, "frozen", False):
            # For PyInstaller onedir, icon is next to the exe
        ...
        for ancestor in [here.parent] + list(here.parents):
            for name in ICON_NAMES:
                candidates.append(str(ancestor / name))
            if (ancestor / "src" / "duga").exists() or (ancestor / "pyproject.toml").is_file():
                break
        for p in candidates:
            if os.path.isfile(p):
                return p
    ...
    def _set_window_icon(self):
        ...
        Uses both iconbitmap and iconphoto because iconphoto is often required
        for the taskbar button to show the custom icon on Windows.
        """
        icon_path = self._find_icon_path()
        if not icon_path:
            return
        try:
            ...
            # iconphoto is more reliable for the Windows taskbar.
            # Use Pillow if available (better .ico support), else PhotoImage.
            icon_img = None
            try:
            ...
            if icon_img is not None:
                ...
                self._icon_image = icon_img  # prevent GC
            ...
            # Help ensure the icon is picked up by the taskbar
            try:
    ...
    def _build_ui(self):
        ...
        self.web_list = EditableList(targets_tab, "Websites (enter as full https:// URLs)", self.targets.get("websites", []),
                                     lambda items: self._update_targets("websites", items))
        self.web_list.pack(fill="x", pady=8)

        social_frame = ttk.LabelFrame(targets_tab, text="Social Handles (enter usernames/handles, NOT full URLs)", padding=6)
        ...
        ttk.Label(keys_tab, text="Credentials (stored only locally in .env). See README for setup instructions.", font=("Segoe UI", 10)).pack(anchor="w", pady=4)
        ...
        for key, label in [
            ...
            ("VISION_BASE_URL", "Vision Base URL (optional)"),
            ("VISION_MODEL", "Vision Model (optional)"),
        ]:
